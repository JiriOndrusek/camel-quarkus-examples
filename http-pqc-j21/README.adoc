...
The `SecurityConfiguration` class registers BouncyCastle JSSE as the primary security provider and configures PQC support:

[source,java]
----
Security.insertProviderAt(new BouncyCastleJsseProvider(), 1);

// Configure TLS named groups to enable X25519MLKEM768
...
1. **Registers BouncyCastle JSSE** as the primary JSSE provider for TLS operations
2. **Enables X25519MLKEM768** as the preferred named group (key exchange algorithm) for TLS 1.3
3. **Maintains compatibility** by including standard elliptic curves (x25519, secp256r1, etc.) as fallback options

The `jdk.tls.namedGroups` system property controls which key exchange algorithms are available during TLS negotiation. By placing `X25519MLKEM768` first, it becomes the preferred choice when both client and server support it.
...
* `/pqc/verify` - Verify actual TLS session parameters
...
Verify the actual TLS session parameters:
...
$ curl --cert target/certs/client-keystore.p12:changeit \
       --cert-type P12 \
       -k \
       https://localhost:8443/pqc/verify
...
Configured Named Groups: X25519MLKEM768, x25519, secp256r1, secp384r1, secp521r1

✓ X25519MLKEM768 is ENABLED in configuration

Note: The actual negotiated key exchange algorithm is not directly
exposed via standard SSLSession API. The negotiation depends on:
1. Server configured named groups (X25519MLKEM768)
2. Client support for X25519MLKEM768
3. TLS 1.3 negotiation process

For detailed verification, enable SSL debug logging:
-Djavax.net.debug=ssl:handshake
----

TIP: To see the actual key exchange negotiation, run the application with SSL debug logging: `java -Djavax.net.debug=ssl:handshake -jar target/quarkus-app/quarkus-run.jar` and look for "NamedGroup" in the handshake output.
...
* BouncyCastle JSSE provider is registered correctly
* TLS named groups are configured with X25519MLKEM768
...
* `/pqc/verify` endpoint confirms PQC configuration is active
* mTLS requirement is enforced (negative test: rejects requests without client certificate)
...
* **PQC Configuration**: X25519MLKEM768 hybrid key exchange is configured via the `jdk.tls.namedGroups` system property in `SecurityConfiguration`. This controls which key exchange algorithms are available during TLS 1.3 negotiation.

* **BouncyCastle Version Consideration**: As of BouncyCastle 1.84 (current version), full X25519MLKEM768 TLS support may still be in development. This example demonstrates the proper configuration approach. If the hybrid algorithm is not recognized, TLS will automatically fall back to standard X25519 key exchange. Check BouncyCastle release notes for the latest PQC TLS support status.

* **Client Support Required**: For X25519MLKEM768 to be used, both server and client must support this hybrid algorithm. When testing with curl or other standard clients, the connection will use X25519 (classical ECDH) until clients add ML-KEM support. Use the `/pqc/verify` endpoint and SSL debug logging to confirm which algorithms are actually negotiated.
