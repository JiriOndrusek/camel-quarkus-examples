...
/**
 * Test RestAssured client with X25519MLKEM768-only server (no fallback).
 * RestAssured uses BCJSSE which supports PQC, so the request succeeds.
 *
 * Run with: mvn test -Dtest=RestAssuredPqcOnlyTest
 */
@QuarkusTest
@TestProfile(PqcOnlyProfile.class)
class RestAssuredPqcOnlyTest extends AbstractRestAssuredPqcTest {
    ...
    protected String getTestDescription() {
        return "RestAssured with PQC-Only Server";
    }
